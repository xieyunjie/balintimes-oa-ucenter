...
define(
		[ 'angularAMD', 'balintimesConstant', 'ui-bootstrap',
				'angular-messages', 'angular-treetable' ],
		function(angularAMD, balintimesConstant) {

			var app = angular.module("postModule", [ 'ui.router',
					'ui.bootstrap', 'ngMessages', 'ngTreetable' ]);

			var mainState = {
				name : 'org/post',
				url : '/org/post',
				templateUrl : balintimesConstant.rootpath
						+ '/views/org/post/list.html',
				...
				resolve : {
					postData : function(AjaxRequest, $stateParams) {
						if ($stateParams.uid == "0") {
							...
							return AjaxRequest.Get("/post/getone/"
									+ $stateParams.uid);
						}
			...
			app.controller(
							...
							function($scope, $state, AjaxRequest, DlgMsg,
								...
								$scope.Deletepost = function(uid) {
									// DlgMsg.confirm("系统提示",
									// "注意！！是否确认删除此职位？此职位删除后，其下属职位也会一拼删除。").result.then(function
									// (btn) {
									// if (btn == "ok") {
									// AjaxRequest.Post("/post/delete", {uid:
									// uid}).then(function () {
									// $scope.initpostTree();
									// })
									// }
					...
					.controller(
							"postEditController",
							function($scope, $state, AjaxRequest,
									TreeSelectModal, DlgMsg, postData, orgData) {
								...
								AjaxRequest.Get("/post/tree").then(
										function(res) {
								...
								$scope.Savepost = function() {
									if ($scope.post.uid == "0") {
										AjaxRequest.Post("/post/create",
												$scope.post).then(
												function(res) {
													$state.go("org/post");
												})
									} else {										
										AjaxRequest.Post("/post/update",
												$scope.post).then(
												function(res) {
													$state.go("org/post");
												})
